...
CITY = CFG.get("city", "尼崎市")
SITE = CFG.get("site_name", "尼崎歯科総研")
DOMAIN = CFG.get("domain", "shikasoken.com")
EN_UPPER = CFG.get("site_name_en", "AMAGASAKI DENTAL RESEARCH")   # 例: AMAGASAKI DENTAL RESEARCH
# 例: Amagasaki Dental Research Institute（site_name_enから機械導出。build_clinics.py と同一ロジック）
EN_INSTITUTE = " ".join("-".join(p.capitalize() for p in w.split("-")) for w in EN_UPPER.split()) + " Institute"
# 区の抽出は都市名から動的生成（例: 尼崎市○区 / 神戸市○区）。区制の無い都市（尼崎市・広域エリア）は
...
def _art_equipment_visibility():
    ...
    def _ward(c):
        m = re.search(r"(尼崎市[^\s0-9０-９]{1,4}区)", c.get("address", ""))
        return m.group(1) if m else None
    wd = {}
    for c in deep:
        w = _ward(c)
    ...
    for w, cs in wd.items():
        if len(cs) >= 25:
            ...
            wrates.append((w.replace("尼崎市", ""), r, len(cs)))
...
<p>当てはまるものが<strong>0〜1個なら、設備名だけで便益が伝わっていない</strong>可能性があります。<strong>2〜3個なら基本はあるが説明に伸びしろ</strong>、<strong>4〜5個なら比較的伝わりやすい構成</strong>です。この記事の数字は{CITY}{nd:,}院の平均像なので、自院の位置を重ねる目安にしてください。</p>
    ...
    return dict(
        ...
        cta=("../../shikumi.html",
             ...
             "設備の便益説明・症例への導線・料金や院長情報の充実度が、同じ区の医院と比べてどうか。尼崎歯科総研は公開情報をもとに分析しています。医院・開業医の方へのご案内はこちら。"),
        body=body, faq=faq,
        method=[
...
<footer class="rp-foot">
  ...
  © {date.today().year} {esc(SITE)}
    ...
    out.parent.mkdir(parents=True, exist_ok=True)  # 新規都市はresearch/未作成のため（尼崎は既存で表面化せず）
    out.write_text(doc, encoding="utf-8")
    return out
