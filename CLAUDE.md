...
## サイト概要
- 対象：尼崎市の歯科医院ポータル。エリアは駅・地区ベース6分類（区なし）
- 設定の正本：`site_config.json`＋`assets/site-config.js`。都市固有の値は必ずここに集約
- 記事生成側の設定：`AI評判設計システム/client_config_amagasaki.json`（daily_post.sh の CLIENT_CONFIG 環境変数で直接指定する。旧コピー差し替え運用は廃止・2026-07-10）
...
- [x] 京都版（修正済みコード）から複製・git init・amagasaki_stations.py・グリッド・AREA_KEYWORDS・ブランド置換
- [x] データ収集（完了：352院収集・掲載209院。公的統計237施設→捕捉率88%）
- [x] 監査→deepdive→slug（衝突0）→座標→最寄駅（全209院）
- [~] サイト生成：医院ページ・一覧は生成済み。**残：AI分析backfill（equipment_stars等が空。429対策後に再実行）→特徴ページ再生成→統計実数化**
...
- [ ] 毎日投稿 launchd（時刻は他都市とずらして 10:30）
