...
CITY = SITE_CFG.get("city", "")                # 例: 尼崎市 / 神戸市 / 北播磨エリア
SITE_NAME = SITE_CFG.get("site_name", "")      # 例: 尼崎歯科総研
EN_UPPER = SITE_CFG.get("site_name_en", "")    # 例: AMAGASAKI DENTAL RESEARCH
# 例: Amagasaki Dental Research Institute（site_name_enから機械導出。ハイフン語は各パートを大文字化）
...
# ファイル名は集約先の実ファイル名（旧・尼崎表記）で固定し、article_href()がローカルに実ファイルが
# あるサイト（尼崎）は相対リンク、無いサイト（複製都市）は集約先への絶対URLを返す。
